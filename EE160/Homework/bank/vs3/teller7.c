...
int main(){
	...
	while(1){
		...
		printf("Deposit(D) Withdrawl (W) Make an new account (M) Proccess An Account(P) Sort(O)\n");
		scanf("%c", &x);

		if(x=='o' || x=='O'){
			sortbankaccount(baa,j);
			sortsavingsaccount(saa,js);
			i = 0;
			for (i = 0;i<j;i++){
				printBankAccount(baa,i);
			}
			for (i = 0;i<js;i++){
				printSavingsAccount(saa,i);
			}
		}
